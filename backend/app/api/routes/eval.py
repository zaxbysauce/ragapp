...
from app.api.deps import get_rag_engine, get_embedding_service
...
class RAGASMetrics(BaseModel):
    ...
    faithfulness: float = Field(0.0, ge=0.0, le=1.0, description="Answer grounded in context")
    answer_relevancy: float = Field(0.0, ge=0.0, le=1.0, description="Answer relevant to query")
    context_precision: float = Field(0.0, ge=0.0, le=1.0, description="Retrieval precision")
    context_recall: float = Field(0.0, ge=0.0, le=1.0, description="Retrieval recall")
    context_relevancy: float = Field(0.0, ge=0.0, le=1.0, description="Context relevance to query")
    answer_similarity: Optional[float] = Field(None, ge=0.0, le=1.0, description="Similarity to ground truth")
...
def _calculate_answer_relevancy(query: str, answer: str) -> float:
    ...
    stop_words = {'the', 'a', 'an', 'is', 'are', 'was', 'were', 'be', 'been', 
                  ...
                  'her', 'its', 'our', 'their', 'mine', 'yours', 'hers',
...
def _calculate_context_precision(contexts: List[str], query: str) -> float:
    ...
    query_words = set(query.lower().split()) - {'the', 'a', 'an', 'is', 'are', 'in', 'of', 'and', 'to'}
...
def _calculate_context_recall(contexts: List[str], ground_truth: Optional[str]) -> float:
    """
    ...
    # Remove stop words
    stop_words = {'the', 'a', 'an', 'is', 'are', 'was', 'were', 'be', 'been',
                  'in', 'of', 'and', 'to', 'for', 'on', 'with', 'at', 'by'}
...
async def ragas_evaluation(
    ...
    try:
        ...
        context_precision = _calculate_context_precision(request.contexts, request.query)
        context_recall = _calculate_context_recall(request.contexts, request.ground_truth)
        context_relevancy = _calculate_context_relevancy(request.contexts, request.query)
        ...
        logger.info(
            ...
            "recall=%.3f, time_ms=%d",
            faithfulness, answer_relevancy, context_precision, context_recall, evaluation_time_ms
        )
