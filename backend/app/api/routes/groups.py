...
async def list_groups(
    ...
    """
    # Cap limit to prevent memory pressure
    if limit > 1000:
        limit = 1000
    # Get total count
...
async def create_group(
    ...
    Requires admin role. User must be a member of the target organization.
    """
    ...
    cursor = await asyncio.to_thread(
        ...
        VALUES (?, ?, ?)
        """,
        (request.org_id, request.name, request.description),
    )
...
async def update_group_members(
    ...
    if request.user_ids:
        ...
        cursor = await asyncio.to_thread(
            db.execute,
            f"SELECT user_id FROM organization_members WHERE org_id = ? AND user_id IN ({placeholders})",
