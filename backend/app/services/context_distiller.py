...
class ContextDistiller:
    ...
    async def distill(
        ...
        try:
            sources = await self._deduplicate(sources, settings.context_distillation_dedup_threshold)
        except Exception as exc:
            logger.warning("Context distillation dedup failed, returning unmodified: %s", exc)
            return sources
